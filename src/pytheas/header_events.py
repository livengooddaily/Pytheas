...
def arithmetic_sequence_non_adjacent(row_values, step_count_k=2):
    ...
    # row_values=['1','2001','2002','2003','2004']
    # sample_symbols={0:[set(['D'])],1:[set(['D'])],2:[set(['D'])], 3:[set(['D'])]}
    step_increment, step_count = utilities.discover_incremental_values_at_least_one_nonadjacent(row_values,
                                                                                                sample_symbols)
    # print('step_increment='+str(step_increment))
    # print('step_count='+str(step_count))
    # print('step_count_k threshold='+str(step_count_k))
...
def collect_events_on_row(row_values):
    ...
    fired = metadata_like_row(
        row_values)  # row has no digits, at least one value enclosed by parenthesis or contains currency sign
    if fired == True:
...
def integer_sequence_adjacent(row_values):
    ...
    if len(row_values) >= 2:
        ...
        for i in range(len(numeric_values) - 1):
            if (numeric_values[i] != None and numeric_values[i + 1] != None and numeric_values[i] + 1 == numeric_values[
                i + 1]):
...
def aggregation_on_row_wo_numeric(row_values):
    values = [str(value).strip().lower() if str(value).strip().lower() not in ['nan', 'none', 'null'] else '' for value
              in row_values]
    ...
    for value in values:
        ...
        is_number = True
        for char in value:
            if char.isdigit() or char == '.' or char == ',' or char == ' ' or (
                    len(value) > 1 and value.startswith('-') and value[1].isdigit()):
                continue
            else:
                is_number = False
                break
...
def aggregation_on_row_w_arith_sequence(row_values, arithmetic_sequence_fired):
    ...
    if arithmetic_sequence_fired == True:
        values = [str(value).strip().lower() if str(value).strip().lower() not in ['nan', 'none', 'null'] else '' for
                  value in row_values]
        for value in values:
            for aggregation_keyword in ['total']:  # , 'average', 'agv', 'mean', 'percentage', '(%)', 'difference'
                if aggregation_keyword in value:
...
def repeating_values_on_row(row_values):
    ...
    if condition_failed == False and repeating_value_seen and max(repeating_lengths) > 0 and max(
            repeating_seen_list) > 1 and len(set(repeating_seen_list)) == 1:
        event_occurred = True
        # print("\n---REPEATING VALUES")
...
def partially_repeating_values_on_row(row_values):
    ...
    for value in value_set:
        if value == None or value.strip().lower() in ['', ' ', 'nan', 'none', 'null'] or value.replace('.', '',
                                                                                                       1).isdigit():
            continue
    ...
    if len(repeating_seen_list) > 0:
        ...
        if condition_failed == False and repeating_value_seen and max(
                repeating_lengths) > 0 and max_repeats >= 2 and repeating_seen_list.count(max_repeats) >= 2:
            event_occurred = True
    else:
        max_repeats = 0
        event_occurred = False
