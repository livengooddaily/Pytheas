...
strictly_null_equivalent = ['nil', 'data not available', 'not available', 'no data', 'no answer', 'nd', 'na', 'n/d',
                            'n/a', '#n/a', 'n.a.', 'not applicable', 'sans objet', 's.o.', 'so', 's/o', '-', '--', '.',
                            '..', '...', '*', 'void', '0', 'redacted', 'confidential', 'confidentiel', 'unknown',
                            'inconnu', '?']
null_equivalent_values = ['', 'null', 'nan', 'none'] + strictly_null_equivalent

footnote_keywords = ['*', 'remarque', 'source', 'note', 'nota', 'not a', 'footnote']
aggregation_tokens = ['total', 'subtotal', 'totaux', 'totales', 'all', 'toute', 'tous', 'less than', 'moins de',
                      'moins que', 'more than', 'plus', 'plus de', 'plusieurs de', 'plus que', 'over', 'under',
                      'higher than', 'plus haut', 'lower than', 'plus bas', 'older than', 'plus vieux', 'younger than',
                      'plus jeune']
...
aggregation_functions = [('totaux', 'sum'), ('totales', 'sum'), ('totale', 'sum'), ('subtotal', 'sum'),
                         ('total partiel', 'sum'), ('total', 'sum'), ('net change in', 'sum'), ('change in', 'sum')
    ...
    "data": {
        "aggregation": {
            "AGGREGATION_TOKEN_IN_FIRST_VALUE_OF_ROW": {
                "name": {
                    "First value of a row (first column) contains an aggregation token, this is likely a summarizing data line"}
            }
        },
        "other": {
...
def normalize_decimals_numbers_predata(value, cand_pattern, cand_symbols, cand_case, cand_num_tokens, cand_num_chars,
                                       summary_patterns, summary_symbols, outlier_sensitive):
    ...
    if 'D' in cand_symbols and cand_symbols.issubset(set(['D', '.', ',', 'S', '-', '+', '~', '(', ')'])):
        ...
        cand_pattern, cand_symbols, cand_case, cand_num_tokens, cand_num_chars = generate_pattern_symbols_and_case(
            new_value, outlier_sensitive)

    for symbols in summary_symbols:
        if (symbols == set() or ('D' in symbols and symbols.issubset(
                set(['D', '.', ',', 'S', '-', '+', '~', '>', '<', '(', ')'])))) == False:
...
def split_metadata_data(csv_tuples, offset, sample_lines_limit, delimiter):
    ...
    while offset < limit:
        ...
        if line in ['\n', '\r\n'] or len(line) == 0 or sum(len(s.strip()) for s in line) == 0:
            while line in ['\n', '\r\n'] or (
                    len(line) == 0 or sum(len(s.strip()) for s in line) == 0) and offset < limit:
...
def assess_candidate_header(datarow):
    ...
    if len(datarow) > 2 and str(datarow[0]).strip().lower() in ['', ' ', 'none', 'nan'] and str(
            datarow[1]).strip().lower() in ['', ' ', 'none', 'nan']:
        candidate = datarow[2::]
    ...
    for idx, value in enumerate(row_values):
        if value != None:
            ...
            try:
                ...
                continue
            if (value.count('-') == 0 and isfloat(value)) or (
                    value.count('-') == 1 and len(sample_symbols[idx]) > 0 and (sample_symbols[idx][0]).issubset(
                    set(['D', ',', '.', '-', 'S'])) and isfloat(evaluate(value))):
                try:
                    ev = evaluate(value)
                except:
                    ev = None
        ...
        for i in range(len(numeric_value_list)):
            ...
            for k in k_indexes:
                ...
                if value - numeric_value_list[k] > 0:
                    ...
                    if step in step_pairs.keys() and numeric_value_list[len(numeric_value_list) - (i + 1)] <= \
                            numeric_value_list[step_pairs[step][-1]] and k < step_pairs[step][-1]:
...
def identify_postmeta_candidates(dataframe):
    ...
    for index, row in dataframe.iloc[::-1].iterrows():
        ...
        if len(datatuple) > 1 and len(datatuple[0]) > 0 and ('source' in (str(datatuple[0]).strip()).lower() or len(
                [x for x in datatuple[1:] if x != None and len(str(x).strip()) > 0]) == 0):
...
def discover_header_and_datastart(org_dataframe, candidate_size=20, trust_threshold=5, max_sample_size=100):
    ...
    if postmeta_cand_startindex < org_dataframe.shape[0]:
        ...
        dataframe = org_dataframe.drop(
            org_dataframe.index[list(range(postmeta_cand_startindex, org_dataframe.shape[0]))])
    ...
    for column in dataframe:
        ...
        for value in columnvalues:
            ...
            pattern, symbols, case, value_num_tokens, value_num_chars = generate_pattern_symbols_and_case(
    ...
    for candidate_index, cand_predata_row in candidate_pre_data_rows.iterrows():
        ...
        if candidate_index == 0:
            for column in dataframe:
                all_values_summary, consistent_symbol_chain = generate_pattern_summary(
                    sample_patterns[dataframe_labels.index(column)])
            ...
            if len(cand_predata_row) > 0:
                ...
                if str(cand_predata_row[dataframe_labels[0]]).strip() != '':
                    ...
                    strong_agreement, agreeinging_columns, data_confidence = strong_pattern_agreement(values_dataframe,
                                                                                                      sample_patterns,
                                                                                                      sample_symbols,
                                                                                                      sample_cases,
                                                                                                      [row_values])
                    ...
                    if strong_agreement:
                        ...
                        if first_data_row_confidence[candidate_index] >= 0.9:
                            outlier_aware_summaries, outliers = generate_outlier_sensitive_column_summaries(
                                values_dataframe)
                            if postmeta_cand_startindex != dataframe.shape[0]:
                                dataend_index = eval_candidate_postmeta(cand_postmeta_rows, outlier_aware_summaries[
                                    0])  # pre_data_row_col_summaries[0]
                                # input('\n\ndataend_index1='+str(dataend_index))
                            else:
                                dataend_index = org_dataframe.shape[0]
                            return datastart_index, dataend_index, header_index, first_data_row_confidence[
                                candidate_index], first_row_data_row_column_summaries, {}, agreeinging_columns, {}, 'data starts from first row in block', null_equi_spotted, sorted_rows, outlier_aware_summaries, outliers
        ...
        for column in dataframe:
            # input('\nCOLUMN_'+str(column))
            summary_strength[dataframe_labels.index(column)] = sum(
                1 for x in sample_patterns[dataframe_labels.index(column)] if len(x) > 0)
            ...
            columns_symbols_summary[dataframe_labels.index(column)] = generate_symbol_summary(
                sample_symbols[dataframe_labels.index(column)])
            columns_cases_summary[dataframe_labels.index(column)] = generate_case_summary(
                sample_cases[dataframe_labels.index(column)])
            ...
            columnsummary = ColumnSampleSummary(columns_pattern_FW_summary[dataframe_labels.index(column)],
                                                columns_pattern_BW_summary[dataframe_labels.index(column)],
                                                columns_chain_consistency[dataframe_labels.index(column)],
                                                columns_symbols_summary[dataframe_labels.index(column)],
                                                columns_cases_summary[dataframe_labels.index(column)],
        ...
        for column in dataframe:
            ...
            if candidate_value == '' or candidate_value == None or str(
                    candidate_value).strip() == '':  # in ['','data not available', 'nan','not available', 'no data', 'no answer', 'nd', 'na', 'n/d', 'n/a','not applicable', '-','--','.', '..','...', 'null', 'none']:
                ...
                if candidate_index > 0 and first_value_on_row_isempty == True:
                    ...
                    if candidate_value == '' or candidate_value == None or str(
                            candidate_value).strip().lower() in null_equivalent_values:
                        continue
                else:
                    continue

            cand_pattern, cand_symbols, cand_case, cand_num_tokens, cand_num_chars = generate_pattern_symbols_and_case(
                candidate_value.strip(), False)

            if (len(sample_patterns[dataframe_labels.index(column)]) > 1 and summary_strength[
                dataframe_labels.index(column)] >= 2) or (
                    len(sample_patterns[dataframe_labels.index(column)]) == 1 and summary_strength[
                dataframe_labels.index(column)] == 1):
                disagreements = find_disagreement(value_pattern_summary, value_pattern_BW_summary,
                                                  value_chain_consistency, cand_pattern, value_symbol_summary,
                                                  cand_symbols, case_summary, cand_case)
                if len(disagreements) > 0:
                    disagreeing_columns[dataframe_labels.index(column)] = {}

                    confidence = disagreement_confidence(disagreements,
                                                         summary_strength[dataframe_labels.index(column)])
        ...
        strong_agreement, agreeing_columns, data_confidence = strong_pattern_agreement(
            dataframe.iloc[candidate_index + 1::, :], sample_patterns, sample_symbols, sample_cases, next_row_values)

        # calculate summary disagreements for this row
        predata_row_confidence = calculate_row_summary_disagreement(disagreeing_columns, candidate_index, row_values,
                                                                    sample_symbols)
        row_agreements[candidate_index + 1] = agreeing_columns
        row_disagreements[candidate_index] = disagreeing_columns

        first_data_row_confidence[candidate_index + 1] = combined_row_confidence(float(predata_row_confidence),
        ...
        input('\n---->Row_' + str(candidate_index + 1) + ' is the first data row with combined confidence: ' + str(
            first_data_row_confidence[candidate_index + 1]))
        ...
        if first_disagreement_found == True and len(first_disagreeing_row.keys()) == 0:
            ...
            first_disagreeing_row["column_summaries"] = [json.dumps(x.__dict__) for x in row_column_summaries[
                candidate_index]]  # row_column_summaries[candidate_index]
        ...
        if data_confidence >= 0.9 or (
                data_confidence + predata_row_confidence) / 2 >= 0.9:  # and assess_candidate_header(candidate_pdr):
            # input('\ndata_confidence>=0.9\n')
        ...
        if candidate_pre_data_rows.shape[0] == 1:
            ...
            for row_index, row_conf in sorted_rows:
                ...
                if row_index - 1 >= 0:
                    ...
                    header_index, header_detection_rule = asses_candidate_predatarow(candidate_pre_data_rows,
                                                                                     row_index - 1)
    ...
    if cand_postmeta_exists:
        dataend_index = eval_candidate_postmeta(cand_postmeta_rows,
                                                outlier_aware_summaries[0])  # pre_data_row_col_summaries[0])
...
def eval_candidate_postmeta(cand_postmeta_rows, pre_data_row_firstcol_summary):
    ...
    for rowindex, row in cand_postmeta_rows.iterrows():
        ...
        disagreements = find_disagreement(pre_data_row_firstcol_summary.fw_pattern_summary,
                                          pre_data_row_firstcol_summary.bw_pattern_summary,
                                          pre_data_row_firstcol_summary.strict_chain_summary,
...
def asses_candidate_predatarow(candidate_pre_data_rows, candidate_index):
    ...
    elif str(candidate_pdr[0]).strip() == '' or str(candidate_pdr[0]).strip() == '' and str(
            candidate_pdr[1]).strip() == '':
        # print('\ntry two row combo')
        if candidate_index - 1 >= 0:
...
def find_disagreement(value_pattern_summary, value_pattern_BW_summary, value_chain_consistent, cand_pattern,
                      value_symbol_summary, cand_symbols, case_summary, cand_case):
    disagreements = []
...
def decimal_pattern(column_symbols, column_patterns):
    ...
    for value_index, value_symbols in enumerate(column_symbols):
        ...
        if len(value_symbols) == 0:
            continue
        if 'D' in value_symbols and count_decimal == 1 and value_symbols.issubset(set(['D', ',', '.', '-', 'S'])) and ((
                                                                                                                               [
                                                                                                                                   x[
                                                                                                                                       0]
                                                                                                                                   for
        ...
        elif ('D' in value_symbols and count_decimal == 1 and value_symbols.issubset(
                set(['D', ',', '.', '-', 'S'])) and (([x[0] for x in column_patterns[value_index]].count(
                '-') == 1 and len(column_patterns[value_index]) > 0 and column_patterns[value_index][0][0] == '-') or (
                                                             [x[0] for x in column_patterns[value_index]].count(
                                                                     '-') == 0))) == False:
            return False
...
def numeric_patterns(column_symbols, column_patterns):
    ...
    for value_index, value_symbols in enumerate(column_symbols):
        ...
        if 'D' in value_symbols and value_symbols.issubset(set(['D', ',', '.', '-', 'S'])) and (([x[0] for x in
                                                                                                  column_patterns[
                                                                                                      value_index]].count(
                '-') == 1 and len(column_patterns[value_index]) > 0 and column_patterns[value_index][0][0] == '-') or (
                                                                                                        [x[0] for x in
        ...
        elif ('D' in value_symbols and value_symbols.issubset(set(['D', ',', '.', '-', 'S'])) and (([x[0] for x in
                                                                                                     column_patterns[
                                                                                                         value_index]].count(
                '-') == 1 and len(column_patterns[value_index]) > 0 and column_patterns[value_index][0][0] == '-') or (
                                                                                                           [x[0] for x
...
def ratio_patterns(column_symbols):
    ...
    for value_symbols in column_symbols:
        ...
        elif ('D' in value_symbols and '%' in value_symbols and value_symbols.issubset(
                set(['D', ',', '.', 'S']))) == False:
            return False
...
def strong_pattern_agreement(values_dataframe, sample_patterns, sample_symbols, sample_cases, row_values, p=0.4, a=0.5,
                             w=0.2):  # , p=0.5, PA=1, PB=10):, a was 0.2
    ...
    for column in values_dataframe:
        ...
        if len(column_patterns) > 0 and column_patterns[0] != []:
            ...
            if (len(column_patterns) > 2 and non_empty_patterns >= 3) or (
                    len(column_patterns) == 2 and non_empty_patterns == 2):
                ...
                if consistent_symbol_chain == True and len(all_values_summary) >= 3 and (
                        len(list(set([i[0] for i in all_values_summary]).difference(['A', 'D', 'S', '_']))) > 0):
                    strong_aggreement = True
                    agreeing_columns[dataframe_labels.index(column)] = {}
                    agreeing_columns[dataframe_labels.index(column)]["confidence"] = float(0.8 * POPULATION_WEIGHT)
                ...
                if consistent_symbol_chain == True and len(all_values_summary) >= 2 and len(
                        [i[1] for i in all_values_summary if i[1] >= 2]) > 0:
                    strong_aggreement = True
                    agreeing_columns[dataframe_labels.index(column)] = {}
                    agreeing_columns[dataframe_labels.index(column)]["confidence"] = float(0.8 * POPULATION_WEIGHT)
                ...
                if consistent_symbol_chain == True and len(all_values_summary) == 1 and all_values_summary[0][
                    0] == 'D' and all_values_summary[0][1] >= 3:
                    strong_aggreement = True
                    agreeing_columns[dataframe_labels.index(column)] = {}
                    agreeing_columns[dataframe_labels.index(column)]["confidence"] = float(0.8 * POPULATION_WEIGHT)
                ...
                if consistent_symbol_chain == True and len(all_values_summary) in [1, 2] and all_values_summary[0][
                    0] == 'D':
                    agreeing_columns[dataframe_labels.index(column)] = {}
                ...
                if numeric_patterns(column_symbols, column_patterns):
                    ...
                    agreeing_columns[dataframe_labels.index(column)]["confidence"] = float(0.7 * POPULATION_WEIGHT)
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"] = 'all values digits, optionally have . or , '
                    continue
                if ratio_patterns(column_symbols):
                    agreeing_columns[dataframe_labels.index(column)] = {}
                    agreeing_columns[dataframe_labels.index(column)]["confidence"] = float(0.7 * POPULATION_WEIGHT)
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"] = 'all values digits with \%,  optionally have . or , '
                    continue
                if len(all_values_summary) >= 3 and 'S' not in [x[0] for x in all_values_summary]:
                    ...
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"] = 'three or above symbols in FW summary that do not contain a Space'
                    continue
                if len(pattern_BW_summary) >= 3 and 'S' not in [x[0] for x in pattern_BW_summary]:
                    agreeing_columns[dataframe_labels.index(column)] = {}
                    agreeing_columns[dataframe_labels.index(column)]["confidence"] = float(0.6 * POPULATION_WEIGHT)
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"] = 'three or above symbols in BW summary that do not contain a Space'
                    continue
                if len(sample_symbols) >= 2 and {'S', '_'}.intersection(sample_symbols) == 0:
                    ...
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"] = "at least two symbols in the symbol summary, none of which are S or _"
                    continue
                ...
                if consistent_symbol_chain == True and len(all_values_summary) in [1, 2]:
                    ...
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"] = 'one or two symbols consistent chain'  # eg, words
                    continue
                ...
                if len(all_values_summary) >= 2 and 'S' not in [x[0] for x in all_values_summary]:
                    ...
                    agreeing_columns[dataframe_labels.index(column)]["confidence"] = float(0.2 * POPULATION_WEIGHT)
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"] = 'two or above symbols in FW summary that do not contain a Space'
                    continue
                if len(pattern_BW_summary) >= 2 and 'S' not in [x[0] for x in pattern_BW_summary]:
                    agreeing_columns[dataframe_labels.index(column)] = {}
                    agreeing_columns[dataframe_labels.index(column)]["confidence"] = float(0.2 * POPULATION_WEIGHT)
                    agreeing_columns[dataframe_labels.index(column)][
                        "rule"] = 'two or above symbols in BW summary that do not contain a Space'
                    continue
...
def calculate_row_summary_disagreement(disagreeing_columns, lineindex, row_values, sample_symbols, a=0.5,
                                       method='precise'):  # a= 0.2
    ...
    if len(column_confidence_values) != 0 and method == 'avg':
        # using average
        predata_row_confidence = 1 - (1 - (
                    sum(column_confidence_values) / len(column_confidence_values))) ** number_of_disagreeing_attributes
...
def disagreement_confidence(disagreements, nonnull_values, p=0.4):  # , p=0.5, PA=1, PB=10):
    ...
    elif len(disagreements) == 1 and (
            disagreements[0] == "FW" or disagreements[0] == "BW" or disagreements[0] == "CC" or disagreements[
        0] == "SS"):
...
def process_file(filepath):
    ...
    if singletable == False:
            ...
            if sum(i > datastart for i in file_blank_rowindex) > 0:
                ...
                if dataend == 0:
                    ...
                    if relation_counter in pre_metafiles.keys():
                        ...
                        pre_metafiles[relation_counter] = {}
                        pre_metafiles[relation_counter]["lines"] = [
                            delimiter.join(x).replace('\n', ' ').replace('\r', '') for x in pre_meta]
                ...
                datastart_index, dataend_index, headerindex, row_confidence, column_summaries, disagreeing_columns, agreeing_columns, first_disagreeing_row, header_detection_rule, null_equi_spotted, sorted_conf, outlier_aware_summaries, outliers = discover_header_and_datastart(
                    df)
                # LOG
                ...
                header["header_text"] = df[headerindex[0]: headerindex[
                    1]].values.tolist()  # csv_tuples[header["from_line_idx"]:header["to_line_idx"]]
                ...
                data["five_lines"] = df[data["from_line_idx"]:min((df.shape[0] - data["from_line_idx"] - 1), 5)].to_csv(
                ...
                datatable = DataTable(premeta, header, data, postmeta, deciding_row, first_disagreeing_row, sorted_conf,
                                      outlier_aware_summaries, outliers)
                relation_counter += 1
                ...
                if singletable and metadataseen == False:
                    ...
                    datatable = DataTable(premeta, header, data, postmeta, deciding_row, first_disagreeing_row,
                                          sorted_conf, outlier_aware_summaries, outliers)
        ...
        datastart_index, _, headerindex, row_confidence, column_summaries, disagreeing_columns, agreeing_columns, first_disagreeing_row, header_detection_rule, null_equi_spotted, sorted_conf, outlier_aware_summaries, outliers = discover_header_and_datastart(
            df)
        ...
        header["header_text"] = df[headerindex[0]: headerindex[
            1]].values.tolist()  # batch[header["from_line_idx"]:header["to_line_idx"]]
        header["header_rule"] = header_detection_rule
        if headerindex[0] != headerindex[1]:
            header["combo"] = combo_row(df[headerindex[0]: headerindex[1]])
            header["attribute_patterns"] = row_patterns(header["combo"])
