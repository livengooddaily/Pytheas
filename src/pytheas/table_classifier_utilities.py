...
def eval_not_data_cell_rule(rule, repetitions_of_candidate, repetitions_of_neighbor, neighbor, value_pattern_summary,
                            value_pattern_BW_summary, value_chain_consistent, value_symbol_summary, case_summary,
                            length_summary, cand_pattern, cand_symbols, cand_case, cand_length, summary_strength,
    ...
    if rule == "First_FW_Symbol_disagrees":
        ...
        if summary_strength > 1 and value_chain_consistent == True and pat.symbol_chain_disagrees(value_symbol_chain,
                                                                                                  cand_symbol_chain):
        ...
        if line_index + 1 in line_agreements.keys() and line_index in line_agreements.keys():
            ...
            if columnindex in next_line_agreements.keys():
                ...
                if line_agreements[line_index][columnindex][
                    'null_equivalent'] == False and rule in value_below_agreements and columnindex in line_agreements[
                    line_index].keys() and rule not in line_agreements[line_index][columnindex]['agreements']:
                    rule_fired = True
...
def eval_data_cell_rule(rule, columnvalues, column_tokens, all_values_summary, consistent_symbol_chain,
                        pattern_BW_summary, value_symbol_summary, column_symbols, column_patterns, case_summary,
                        candidate_count, partof_multiword_value_repeats, candidate_tokens, consistent_symbol_set,
    ...
    if candidate == None or str(candidate).strip().lower() in null_equivalent:
        ...
        rule_fired = False
        if consistent_symbol_chain and len(value_symbol_summary) == 1 and value_symbol_summary[
            0] == 'A' and case_summary in ['ALL_CAPS', 'ALL_LOW', 'TITLE']:
            rule_fired = True
        ...
        rule_fired = False
        if consistent_symbol_chain == True and len(all_values_summary) == 1 and all_values_summary[0][0] == 'D' and \
                all_values_summary[0][1] > 0:
            rule_fired = True
        ...
        rule_fired = False
        if consistent_symbol_set and len(
                value_symbol_summary) >= 2 and 'S' not in value_symbol_summary and '_' not in value_symbol_summary:
            rule_fired = True
...
def eval_numeric_pattern(train):
    ...
    if 'D' in symbolset and symbolset.issubset(set(['D', ',', '.', '-', 'S'])) and (
            ([x[0] for x in train].count('-') == 1 and len(train) > 0 and train[0][0] == '-') or (
            [x[0] for x in train].count('-') == 0)):
        return True
    else:
        return False
...
class TableSignatures:
    def __init__(self, dataframe=pd.DataFrame(), outlier_sensitive=False):
        if dataframe.empty == False:
            ...
            is_aggregate = tokens.applymap(
                lambda cell: (len(cell) > 0 and not set(cell).isdisjoint(pat.aggregation_tokens)))
...
def discover_aggregation_scope(csv_file, aggregation_rows, cand_subheaders, predicted_subheaders, certain_data_indexes,
                               pat_headers):
    subheader_scope = {}
    ...
    if len(aggregation_rows) > 0 and len(certain_data_indexes) > 0:
        ...
        if len(agg_idxs) > 0 and agg_idxs[0] != certain_data_indexes[0]:
            ...
            for aggregation_idx in agg_idxs:
                ...
                candidate_scope = csv_file.loc[[i for i in sorted(certain_data_indexes + cand_subhead_indexes) if
                                                i < aggregation_idx and i not in agg_idxs]].applymap(normalize_numeric)
                ...
                if aggregation_function == 'sum':
                    for i in range(1, candidate_scope.shape[0] + 1):
                        ...
                        summed_rows = candidate_scope.loc[candidate_scope.index[-i:]].sum(axis=0,
                                                                                          skipna=True)  # .replace({ 0:np.nan})
                        ...
                        if scope_head_idx != None:
                            ...
                            if cand_scope_head in candidate_scope.index:
                                summed_rows = candidate_scope.loc[cand_scope_head:].sum(axis=0,
                                                                                        skipna=True)  # .replace({ 0:np.nan})
                            # print(f'\n\tsummed_rows1=\n{pd.DataFrame([summed_rows])}')

                            while cand_scope_head >= candidate_scope.index[0] and aggregation_row.iloc[0].eq(
                                    summed_rows).any():
                                # print(f'\ncandidate_scope = \n{candidate_scope}')
                                if cand_scope_head in candidate_scope.index:
                                    summed_rows = candidate_scope.loc[cand_scope_head:].sum(axis=0,
                        ...
                        if scope_head_idx != None:
                            ...
                            aggregation_rows[aggregation_idx]['scope_range'] = list(
                                range(scope_head_idx, aggregation_idx))
                            ...
                            for cand_subheader_idx in cand_subheader_rev:
                                ...
                                if value.lower() in aggregation['aggregation_label'].lower():
                                    ...
                                    aggregation_rows[aggregation_idx]['scope_range'] = list(
                                        range(cand_subheader_idx + 1, aggregation_idx))
                                    # input(aggregation_rows[aggregation_idx])
                                    break

                            if 'context_label' not in aggregation_rows[
                                aggregation_idx].keys() and last_header_value != '' and last_header_value.lower() in \
                                    aggregation["aggregation_label"].lower():
                                ...
                                aggregation_rows[aggregation_idx]['scope_range'] = list(
                                    range(pat_headers[-1] + 1, aggregation_idx))
                            # input(aggregation_rows[aggregation_idx])

                            if 'subheader' not in aggregation_rows[
                                aggregation_idx].keys() and cand_subhead_indexes != None and \
                                    aggregation_rows[aggregation_idx]['scope_head'] - 1 in cand_subhead_indexes:
                                # input(f'prediction is missing subheader, look for a subheader')
                                aggregation_rows[aggregation_idx]['subheader'] = aggregation_rows[aggregation_idx][
                                                                                     'scope_head'] - 1
                                subheader_scope[aggregation_rows[aggregation_idx]['scope_head'] - 1] = list(
                                    range(aggregation_rows[aggregation_idx]['scope_head'], aggregation_idx))
                                ...
                                aggregation_rows[aggregation_idx]['context_label'] = \
                                csv_file.loc[aggregation_rows[aggregation_idx]['scope_head'] - 1].tolist()[0]
                            ...
                            if 'context_label' not in aggregation_rows[aggregation_idx].keys():
                                ...
                                for cand_subheader_idx in cand_subheader_rev:
                                    ...
                                    if value.lower() in aggregation['aggregation_label'].lower():
                                        aggregation_rows[aggregation_idx]['context_label'] = value
                                        aggregation_rows[aggregation_idx]['subheader'] = cand_subheader_idx
                                        subheader_scope[cand_subheader_idx] = list(
                                            range(scope_head_idx, aggregation_idx))
                                        predicted_subheaders.append(cand_subheader_idx)
                                        cand_subheaders.pop(cand_subheader_idx)
                                        cand_subhead_indexes.remove(cand_subheader_idx)
                        ...
                        if cand_subhead_indexes != None and candidate_scope.index[-i] - 1 in cand_subhead_indexes:
                            ...
                            if candidate_subheader_value.lower() in aggregation['aggregation_label'].lower():
                                ...
                                if scope_head_idx - 1 in cand_subhead_indexes:
                                    ...
                                    certain_data_indexes = list(set(certain_data_indexes) - set(
                                        aggregation_rows[aggregation_idx]['scope_range']))
                                ...
                                aggregation_rows[aggregation_idx]['context_label'] = \
                                csv_file.loc[scope_head_idx - 1].tolist()[0]
                                # print(f'aggregation_rows[{aggregation_idx}]=')
                                # pp.pprint(aggregation_rows[aggregation_idx])
                                break
...
def assess_non_data_line(row_values, before_data, all_summaries_empty, row_index, dataframe, min_left_non_null=1):
    ...
    if row_index + 1 != dataframe.shape[0] and len(row_values) > 0 and row_values[0].lower() in ['', 'none', 'nan']:
        if len(row_values) > 1:
            ...
            for value_index, value in enumerate(row_values[1:]):
                column = dataframe.iloc[row_index:min(row_index + 5, dataframe.shape[0]),
                         value_index + 1:value_index + 2]
    ...
    if len(row_values) > 0 and row_values[0].lower() not in ['', 'none', 'nan'] and len(
            [i for i in row_values if i.lower() not in ['', 'none', 'nan']]) <= 2:
...
def predict_line_confidences(dataframe, dataframe_patterns, args, dataframe_labels, groundtruth_key, line_agreements):
    ...
    for row_index, candidate_row in dataframe.iterrows():
        ...
        for column in dataframe:
            ...
            if row_counter > 0 and (line_agreements[row_index][columnindex]['null_equivalent'] == True or
                                    line_agreements[row_index][columnindex][
                                        'summary_strength'] == 1) and impute_nulls == True and row_index - 1 in line_agreements.keys() and columnindex in \
                    line_agreements[row_index - 1].keys() and row_index - 1 in data_line_confidences.keys() and \
            ...
            if row_counter > 0 and line_agreements[row_index][columnindex]['summary_strength'] == 0 and \
                    line_agreements[row_index][columnindex][
                        'aggregate'] and row_index - 2 in line_agreements.keys() and columnindex in line_agreements[
            ...
            cand_pattern, cand_symbols, cand_case, cand_num_tokens, cand_num_chars = pat.generate_pattern_symbols_and_case(
                candidate_value.strip(), outlier_sensitive)
            ...
            if normalize_decimals == True:
                cand_pattern, cand_symbols, cand_case, cand_num_tokens, cand_num_chars, column_patterns, column_symbols = pat.normalize_decimals_numbers_predata(
                    candidate_value.strip(), cand_pattern, cand_symbols, cand_case, cand_num_tokens, cand_num_chars,
                    column_patterns, column_symbols, outlier_sensitive)
            ...
            if len(cand_pattern) > 0:
                ...
                if disagreement_summary_strength > 0 and (
                        all_numbers(column_symbols) == False or is_number(cand_symbols) == False):
                    value_disagreements = find_disagreements(repetitions_of_candidate, repetitions_of_neighbor,
                                                             neighbor, value_pattern_summary, value_pattern_BW_summary,
                                                             value_chain_consistent, value_symbol_summary, case_summary,
                                                             length_summary, cand_pattern, cand_symbols, cand_case,
                                                             cand_num_chars, disagreement_summary_strength)
            ...
            if row_index + 1 in line_agreements.keys() and columnindex in line_agreements[row_index + 1].keys():
                ...
                for agreement in value_below_agreements:
                    if line_agreements[row_index][columnindex]['null_equivalent'] == False and \
                            line_agreements[row_index][columnindex]['aggregate'] == False and agreement not in \
                            line_agreements[row_index][columnindex][
                                'agreements'] and agreement in not_data_value_rules.keys():
        ...
        if len(candidate_row_values) > 0 and weight_input == 'values_and_lines':
            ...
            for header_event in header_events_fired:
                ...
                if not_data_weights[header_event] != None and not_data_weights[
                    header_event] > not_data_weight_lower_bound:
                    line_not_data_evidence.append(not_data_weights[header_event])
            if len(arithmetic_events_fired) > 0:
                arithmetic_event_score = max_score(arithmetic_events_fired, not_data_weights,
                                                   not_data_weight_lower_bound)
            ...
            if dataframe.shape[1] > 1:
                not_data_line_rules_fired = assess_non_data_line(candidate_row_values, before_data, all_summaries_empty,
                                                                 row_counter, dataframe)
...
def find_disagreements(repetitions_of_candidate, repetitions_of_neighbor, neighbor, value_pattern_summary,
                       value_pattern_BW_summary, value_chain_consistent, value_symbol_summary, case_summary,
                       length_summary, cand_pattern, cand_symbols, cand_case, cand_length, summary_strength,
                       original_value=True):
    disagreements = []
...
def find_agreements(max_line_depth, columnvalues, column_tokens, all_values_summary, consistent_symbol_chain,
                    pattern_BW_summary, value_symbol_summary, column_symbols, column_patterns, case_summary,
                    max_values_lookahead=6):
    ...
    if len(columnvalues) > 2 and partof_multiword_value_repeats_once(candidate_tokens, column_tokens[
                                                                                       2:min(max_values_lookahead,
                                                                                             len(columnvalues))]):
        # Only one alphabetic token from multiword value repeats below, and it repeats only once
        agreements.append('Rule_2_a')

    if len(columnvalues) > 2 and partof_multiword_value_repeats_at_least_k(candidate_tokens, column_tokens[
                                                                                             2:min(max_values_lookahead,
                                                                                                   len(columnvalues))],
                                                                           2):
...
def find_line_agreements(dataframe, dataframe_patterns, args, start_index, do_ignore_left):
    ...
    for row_index, cand_row in dataframe.iterrows():
        ...
        for column in dataframe:
            ...
            line_agreements[row_index][columnindex]['aggregate'] = (
                        len(value_tokens) > 0 and not set(value_tokens).isdisjoint(pat.aggregation_tokens))
            ...
            if len(column_patterns) > 0 and value_lower not in null_equivalent:
                ...
                if len(column_patterns) >= 2 and non_empty_patterns >= 2:
                    ...
                    agreements = find_agreements(max_line_depth, columnvalues, column_tokens, value_pattern_summary,
                                                 value_chain_consistent, value_pattern_BW_summary, value_symbol_summary,
                                                 column_symbols, column_patterns, case_summary)
...
def discover_delimiter(filepath, encoding):
    ...
    if len(delim_count.values()) > 0:
        ...
        if len(cand_delims) == 0:
            ...
            # print('Detected delimiter = "'+delimiter+'"')
            return delimiter
        if delimiter_idx_of_first_consistent_row[cand_delims[0]] < delimiter_idx_of_first_consistent_row[
            cand_delims[1]]:
            delimiter = cand_delims[0]
            # print('Detected delimiter = "'+delimiter+'"')
            return delimiter
        elif delimiter_idx_of_first_consistent_row[cand_delims[0]] > delimiter_idx_of_first_consistent_row[
            cand_delims[1]]:
...
def match_sequence(legal_label_sequence, line_predictions, offset, first_double_data_index, data_window,
                   label_weight='confidence'):
        ...
        if len(idx) > 0:
            ...
            if idx[0] == dataframe.index[-1]:
                ...
                for row in nans.itertuples():
                    ...
                    if len(first_value) > 5 and (((first_value[0].isdigit() or first_value[0] == 'a') and first_value[
                        1] in [' ', '.', '/', ')', ']', ':']) or (first_value[0] == '(' and (
            ...
            if len(row_values) > 0:
                ...
                for aggregation_phrase in pat.aggregation_functions:  # ['total', 'average', 'subtotal']
                    if first_value.lower().startswith(aggregation_phrase[0]):
                        predicted_last_data_line_found = True
                        break
                if len(row_values) > 2 and len([i for i in row_values[:-1] if
                                                str(i).strip().lower() not in ['', 'nan', 'none',
                                                                               'null']]) == 0 and str(
                        row_values[-1]).strip().lower() not in ['', 'nan', 'none', 'null']:
                    continue
...
def predict_fdl(dataframe, line_predictions, markov_approximation_probabilities, markov_model, data_window=2,
                combined_label_weight='confidence'):
    ...
    if len(line_predictions) > 1:
        ...
        ordered_prediction_labels = [line_predictions[line_index]['label'] for line_index in
                                     sorted(line_predictions.keys()) if line_index >= offset]
        ...
        if first_data_window_index >= 0:
                    ...
                    if markov_model != None:
                        if len(legal_sequences[sequence_id]) == 1:
                            if markov_model == 'first_order':
                                sequence_prior = sequence_prior * markov_approximation_probabilities[
                                    'p_first_tables_start_not_data']
                    ...
                    if markov_model != None:
                        if len(legal_sequences[sequence_id]) == 1:
                            ...
                            if markov_model == 'first_order' and legal_sequences[sequence_id].count('DATA') == 1:
                                sequence_prior = sequence_prior * markov_approximation_probabilities[
                                    'prob_dI_given_ndIminus1']

                            elif markov_model == 'second_order' and len(legal_sequences[sequence_id]) == 2 and \
                                    legal_sequences[sequence_id].count('DATA') == 1:
                                sequence_prior = sequence_prior * markov_approximation_probabilities[
                                    'prob_d_given_start_nd']
                                # print(f"markov_approximation_probabilities['prob_d_given_start_nd']={markov_approximation_probabilities['prob_d_given_start_nd']}")
                                    ...
                                    legal_sequences[sequence_id].count('DATA') == 2:
                                sequence_prior = sequence_prior * markov_approximation_probabilities[
                                    'prob_d_given_start_d']
                                ...
                                # print(f"markov_approximation_probabilities['prob_d_given_nd_d']={markov_approximation_probabilities['prob_d_given_nd_d']}")
                                    ...
                                    legal_sequences[sequence_id].count('DATA') >= 3:
            ...
            match_softmax = {}
            for sequence_id, legal_label_sequence in legal_sequences.items():
                match_softmax[sequence_id] = math.exp(match_weight[sequence_id]) / sum(
                    [math.exp(v) for v in match_weight.values()])
            ...
            for sequence_id, sequence in legal_sequences.items():
                sequence_final_probability[sequence_id] = legal_sequence_priors[sequence_id] * match_softmax[
                    sequence_id]
...
def predict_fdl_old(dataframe, data_confidence, not_data_confidence, combined_data_line_confidences, line_predictions,
                    x=0.6):
    ...
    for row_index, candidate_data_row in dataframe.iterrows():
        ...
        if row_index in combined_data_line_confidences.keys() and combined_data_line_confidences[row_index] > 0:
            if len(candidate_data_row) == sum(
                    [1 for i in candidate_data_row if str(i).strip().lower() in ['', 'nan']]) == 0:
                continue

            first_data_line = row_index
            # if next line is not data
            if row_index + 1 in combined_data_line_confidences.keys() and combined_data_line_confidences[
                row_index + 1] == 0:
...
def last_data_line_confidence(line_predictions, predicted_boundary, max_window=4):
    ...
    for method in ['confidence']:
        ...
        if (data_window_weight + not_data_window_weight) > 0:
            avg_confidence[method] = max(0, (data_window_weight * (avg_predicted_data) + not_data_window_weight * (
                avg_predicted_not_data)) / (data_window_weight + not_data_window_weight))
        else:
    ...
    for method in ['confusion_index', 'confidence', 'difference']:
        ...
        if first_data_line != -1:
            ...
            if (data_window_weight + not_data_window_weight) > 0:
                avg_confidence[method] = max(0, (data_window_weight * (avg_predicted_data) + not_data_window_weight * (
                    avg_predicted_not_data)) / (data_window_weight + not_data_window_weight))
                min_confidence[method] = max(0, (data_window_weight * (min_predicted_data) + not_data_window_weight * (
                    min_predicted_not_data)) / (data_window_weight + not_data_window_weight))
            else:
                avg_confidence[method] = 0
...
def assess_combo_header(candidate_header_dataframe):
    ...
    if len([i for i in candidate_header if str(i).strip().lower() not in ['', 'nan', 'none', 'null']]) == len(
            candidate_header):
        # input('>1')
        return True
    else:
        ...
        if len(candidate_header) == 3:
            if len([i for i in candidate_header[1:] if
                    str(i).strip().lower() not in ['', 'nan', 'none', 'null']]) == len(candidate_header[1:]):
                # input('>3')
                return True
            else:
                return False
        ...
        if len(candidate_header) == 4:
            if len([i for i in candidate_header[2:] if
                    str(i).strip().lower() not in ['', 'nan', 'none', 'null']]) == len(candidate_header[2:]):
                # input('>4a')
                return True
            elif len([i for i in candidate_header[:-2] if
                      str(i).strip().lower() not in ['', 'nan', 'none', 'null']]) == len(candidate_header[:-2]):
                # input('>4b')
                return True
            elif len([i for i in candidate_header[1:-1] if
                      str(i).strip().lower() not in ['', 'nan', 'none', 'null']]) == len(candidate_header[1:-1]) or len(
                    [i for i in candidate_header[1:] if
                     str(i).strip().lower() not in ['', 'nan', 'none', 'null']]) == len(candidate_header[1:]) or len(
                    [i for i in candidate_header[:-1] if
                     str(i).strip().lower() not in ['', 'nan', 'none', 'null']]) == len(candidate_header[:-1]):
                # input('>4b')
                return True
        ...
        if len(candidate_header) > 4 and len([i for i in candidate_header[2:-2] if
                                              str(i).strip().lower() not in ['', 'nan', 'none', 'null']]) == len(
                [i for i in candidate_header[2:-2]]):
            # input('5')
            return True
        else:
            return False
...
def name_table_columns(rows):
    rows = rows.applymap(
        lambda v: pat.dequote(str(v).strip()) if str(v).strip().lower() not in ['', 'nan', 'none', 'null'] else '')
    column_names = {}
    ...
    if candidate_headers.shape[0] > 0:
        ...
        for reverse_index in range(1, candidate_headers.shape[0] + 1):
            ...
            if len([i for i in row_values if str(i).strip().lower() not in ['', 'nan', 'none', 'null']]) == 0:
                ...
                if NON_DUPLICATE_HEADER_ACHIEVED == True:
                    ...
                    continue
            if reverse_index > 1 and len(row_values) > 1 and str(row_values[0]).strip().lower() not in ['', 'nan',
                                                                                                        'none',
                                                                                                        'null'] and len(
                    [i for i in row_values[1:] if str(i).strip().lower() not in ['', 'nan', 'none', 'null']]) == 0:
            ...
            if len(row_values) > 2 and str(row_values[1]).strip().lower() not in ['', 'nan', 'none', 'null'] and len(
                    [i for i in row_values[2:] if str(i).strip().lower() not in ['', 'nan', 'none', 'null']]) == 0:
                if NON_DUPLICATE_HEADER_ACHIEVED == True:
                    break
                else:
                    continue
            ...
            if NON_DUPLICATE_HEADER_ACHIEVED == True and reverse_index > 1 and len(row_values) > 2:
                ...
                for value_index, value in enumerate(row_values[2:]):
                    if str(value).strip().lower() not in ['', 'nan', 'none', 'null'] and str(
                            candidate_headers.iloc[-reverse_index + 1].tolist()[value_index + 2]).strip().lower() in [
                        '', 'nan', 'none', 'null']:
                        if len(row_values) > 4 and (
                                value_index == len(row_values[2:]) - 1 or value_index == len(row_values[2:]) - 2):
                            continue
                        extension = False
                        break
                if extension == True:
        ...
        if len(predicted_header_indexes) > 0:
            if predicted_header_indexes[0] - 1 in dataframe.index:
                ...
                if len(row_before) > 1 and len(
                        [i for i in row_before[1:] if str(i).strip().lower() not in ['', 'nan', 'none', 'null']]) > 0:
            ...
            if len(predicted_header_indexes) > 0:
                ...
                while len(predicted_header_indexes) > 0:
                    ...
                    if len(dataframe.columns) > 1 and file_dataframe.loc[last_header_index,
                                                      1:].isnull().values.all() == True and (
                            first_value.startswith('(') and first_value.endswith(')')) == False:
            ...
            if len(predicted_header_indexes) == 0 and table_counter == 1 and first_data_line_annotated > 0 and len(
                    candidate_headers) > 0:
                count_offset = 0
                for reverse_index in range(1, candidate_headers.shape[0] + 1):
                    count_offset += 1
...
def discover_next_table_temp(csv_file, file_offset, table_counter, line_agreements, line_disagreements, blank_lines,
                             args):
    ...
    for row_index in csv_file.index:
        ...
        for column_index in csv_file.columns:
            ...
            # and line before me exists and was data
            # see impute agreements
            if row_index in line_agreements.keys() and (
                    line_agreements[row_index][column_index]['null_equivalent'] == True or
                    line_agreements[row_index][column_index][
                        'summary_strength'] == 1) and args.impute_nulls == True and row_index - 1 in line_agreements.keys() and column_index in \
                    line_agreements[row_index - 1].keys() and row_index - 1 in data_line_confidences.keys() and \
                    data_line_confidences[row_index - 1] > not_data_line_confidences[row_index - 1]:
            ...
            if row_index in line_agreements.keys() and line_agreements[row_index][column_index][
                'summary_strength'] == 0 and line_agreements[row_index][column_index][
                'aggregate'] and row_index - 2 in line_agreements.keys() and column_index in line_agreements[
                row_index - 2].keys() and row_index - 2 in data_line_confidences.keys() and data_line_confidences[
                row_index - 2] > not_data_line_confidences[row_index - 2]:
            ...
            not_data_score = max_score(value_disagreements, rule_weights['not_data_cell'],
        ...
        if args.weight_input == 'values_and_lines':
            ...
            if row_index > 0 and row_index - 1 in data_line_confidences.keys() and data_line_confidences[
                row_index - 1] > not_data_line_confidences[row_index - 1]:
                before_data = False
            ...
            if csv_file.shape[1] > 1:
                ...
                for event in not_data_line_rules_fired:
                    if event == "UP_TO_FIRST_COLUMN_COMPLETE_CONSISTENTLY" and before_data == False:
                        continue
                    if event.startswith("ADJACENT_ARITHMETIC_SEQUENCE") and rule_weights['not_data_line'][
                        event] == None:
                        steps = event[-1]
                        if int(steps) in range(2, 6):
                            event = event[:-1] + str(int(steps) + 1)
                    if rule_weights['not_data_line'][event] != None and rule_weights['not_data_line'][
                        event] > args.not_data_weight_lower_bound:
        ...
        if args.weight_input == 'values_and_lines':
            ...
            for rule in line_is_data_events:
                if rule_weights['data_line'][rule] != None and rule_weights['data_line'][
                    rule] > args.weight_lower_bound:
    ...
    pat_first_data_line, first_data_line_combined_data_predictions = predict_fdl(csv_file, data_line_confidences,
                                                                                 not_data_line_confidences,
                                                                                 combined_data_line_confidences,
                                                                                 line_predictions,
                                                                                 args.markov_approximation_probabilities,
                                                                                 args.markov_model, 2,
                                                                                 args.combined_label_weight)
...
def process_dataframe(table_counter, input_dataframe, args, delimiter):
    ...
    if pat_first_data_line != -1:
        ...
        predicted_table['fdl_confidence']["avg_majority_confidence"] = float(
            first_data_line_combined_data_predictions["avg_confidence"]['confidence'])
        predicted_table['fdl_confidence']["avg_difference"] = float(
            first_data_line_combined_data_predictions["avg_confidence"]['difference'])
        predicted_table['fdl_confidence']["avg_confusion_index"] = float(
            first_data_line_combined_data_predictions["avg_confidence"]['confusion_index'])
        ...
        while last_data_line_index in combined_data_line_confidences.keys() and line_predictions[last_data_line_index][
            'label'] == 'DATA':
        ...
        predicted_header_indexes, predicted_pat_subheaders = predict_header_indexes(
            file_dataframe.loc[:last_data_line_index], pat_first_data_line, table_counter)
        ...
        cleaned_candidate_data = input_dataframe.loc[pat_first_data_line:last_data_line_index, :].dropna(axis=0,
                                                                                                         how='all',
                                                                                                         inplace=False)
        if len(predicted_header_indexes) > 0:
            ...
            cleaned_candidate_section = input_dataframe.loc[predicted_header_indexes[0]:last_data_line_index, :].dropna(
                axis=1, how='all', inplace=False)
            predicted_table['header']['combo'] = combo_row(file_dataframe.loc[predicted_header_indexes, :])
            predicted_table['five_lines'] = cleaned_candidate_section.loc[
                                            pat_first_data_line:min(cleaned_candidate_section.index[-1],
                                                                    pat_first_data_line + 5), :].to_csv(sep=',',
                                                                                                        index=False,
                                                                                                        header=False)
        else:
            cleaned_candidate_section = cleaned_candidate_data.dropna(axis=1, how='all', inplace=False)
            predicted_table['five_lines'] = cleaned_candidate_section.iloc[:min(cleaned_candidate_section.shape[0], 5),
        ...
        if cleaned_candidate_data.shape[0] == 1 and len(predicted_header_indexes) == 0 or \
                cleaned_candidate_section.shape[1] == 1:
            # print('Invalid Data')
            table_counter -= 1
        else:
            if len(predicted_header_indexes) > 0:
            ...
            if (len(predicted_header_indexes) == 0 and pat_first_data_line != 0) or (
                    len(predicted_header_indexes) > 0 and top_context_boundary != predicted_header_indexes[0]):
...
def view_dataframe(filepath):
    ...
    all_csv_tuples, discovered_delimiter, discovered_encoding, encoding_language, encoding_confidence, failure, blank_lines_index, google_detected_lang = sample_file(
        filepath, 100)
    dataframes = []
    ...
    if failure == None and all_csv_tuples != None and len(all_csv_tuples) > 0:
        ...
        for dataframe in dataframes:
            ...
            try:
                ...
                table_counter, tables, predicted_combined_data = process_dataframe(table_counter, dataframe, args,
