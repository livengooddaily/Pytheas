...
from pytheas.table_classifier_utilities import TableSignatures, is_consistent_symbol_sets, predict_fdl, \
    predict_line_label, predict_combined_data_confidences, predict_header_indexes, eval_data_cell_rule, \
    eval_not_data_cell_rule, line_has_null_equivalent, all_numbers, is_number, assess_data_line, assess_non_data_line, \
    discover_aggregation_scope, contains_number, name_table_columns
import pytheas.pat_utilities as pat_util
from pytheas.header_events import collect_events_on_row, collect_arithmetic_events_on_row, \
...
def generate_processing_tasks(pytheas_model, db_cred, files, max_lines, top_level_dir, opendata_engine):
    ...
    for file in files:
        ...
        filepath = os.path.join(top_level_dir,
                                ground_truth_path)
        task = (
        db_cred, file_counter, pytheas_model, crawl_datafile_key, endpoint, filepath, max_lines, opendata_engine)
        yield task
...
def message_slack(message):
    ...
    data = {"text": message}
    requests.post('https://hooks.slack.com/services/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', headers=headers,
                  data=str(data))
...
def process_file_worker(t):
    ...
    try:
        ...
        if failure == None:
            try:
                with codecs.open(filepath, 'rU', encoding=discovered_encoding) as f:
                    chunk = f.read()
                    if chunk:
                        for line in csv.reader(chunk.split("\n"), quotechar='"', delimiter=discovered_delimiter,
                    ...
                    if predictions != None:
                        ...
                        if num_tables > 0:
                            first_table_fdl_confidence = float(
                                round(predictions[1]["fdl_confidence"]['avg_majority_confidence'], 4))
        ...
        if traceback:
            traceback_str = str(traceback.format_exc())
        print(
            f'[Exception] on  file_counter_{file_counter}: crawl_datafile_key = {crawl_datafile_key} produced exception {e} \n{traceback_str}')
        ...
        with conn.begin():
            ...
            if predictions != None:
                ...
                for table_id in predictions:
                    ...
                    column_names = name_table_columns(table_dataframe.loc[
                                                          header_lines])  # apply combination algorithm to values in discovered header section
                    ...
                    first_data_line_avg_majority_confidence = float(
                        round(table["fdl_confidence"]['avg_majority_confidence'], 4))
                    first_data_line_avg_difference = float(round(table["fdl_confidence"]['avg_difference'], 4))
                    first_data_line_avg_confusion_index = float(
                        round(table["fdl_confidence"]['avg_confusion_index'], 4))
...
class API(object):
    ...
    def learn_and_save_weights(self, files_path, annotations_path, output_path='train_output.json', parameters=None):
        ...
        annotations = sorted(
            [os.path.join(annotations_path, f) for f in listdir(annotations_path) if isfile(join(annotations_path, f))])
        ...
        with Pool(processes=NPROC) as pool:
            with tqdm(total=NINPUTS) as pbar:
                for r in pool.imap_unordered(self.real_pytheas.rules_fired_in_file,
                                             [(key, f[0], f[1]) for key, f in enumerate(zip(files, annotations))]):
...
class PYTHEAS:
    ...
    def __init__(self):
        self.parameters = DotMap({
            ...
            "max_attributes": 20,
            # cuttof for columns to be considered (from left to right) when collecting class confidence
            "outlier_sensitive": True,
            "normalize_decimals": True,
            "impute_nulls": True,
            "ignore_left": 4,
            # if there are enough columns, ignore the first ignore_left when evaluating column class confidence (avoids taking into account what is often referred to as index columns or left headers.)
    ...
    def connect_opendata_profile(self, db_cred):
        self.opendata_engine = create_engine(
    ...
    def train_rules(self, undersampled_cell_data, undersampled_cell_not_data, undersampled_line_data,
                    undersampled_line_not_data):
        ...
        for rule in self.fuzzy_rules["cell"]["data"].keys():
            ...
            if rule.lower() in undersampled_cell_data.columns:
                ...
                data_cell_true_positive = undersampled_cell_data.query(f"{rule.lower()}==True and label=='DATA'").shape[
                    0]
                data_cell_false_positive = \
                undersampled_cell_data.query(f"{rule.lower()}==True and label!='DATA'").shape[0]
        ...
        for rule in self.fuzzy_rules["cell"]["not_data"].keys():
            ...
            if rule.lower() in undersampled_cell_not_data.columns:
                ...
                not_data_cell_true_positive = \
                undersampled_cell_not_data.query(f"{rule.lower()}==True and label!='DATA'").shape[0]
                not_data_cell_false_positive = \
                undersampled_cell_not_data.query(f"{rule.lower()}==True and label=='DATA'").shape[0]
            ...
            if not_data_cell_predicted_positive != 0:
                rule_weight = not_data_cell_true_positive / not_data_cell_predicted_positive - (
        ...
        for rule in self.fuzzy_rules["line"]["data"].keys():
            ...
            if rule.lower() in undersampled_line_data.columns:
                ...
                data_line_true_positive = undersampled_line_data.query(f"{rule.lower()}==True and label=='DATA'").shape[
                    0]
                data_line_false_positive = \
                undersampled_line_data.query(f"{rule.lower()}==True and label!='DATA'").shape[0]
        ...
        for rule in self.fuzzy_rules["line"]["not_data"].keys():
            ...
            if rule.lower() in undersampled_line_not_data.columns:
                ...
                not_data_line_true_positive = \
                undersampled_line_not_data.query(f"{rule.lower()}==True and label!='DATA'").shape[0]
                not_data_line_false_positive = \
                undersampled_line_not_data.query(f"{rule.lower()}==True and label=='DATA'").shape[0]
            ...
            if not_data_line_predicted_positive != 0:
                rule_weight = not_data_line_true_positive / not_data_line_predicted_positive - (
    ...
    def process_endpoint(self, endpoint,
        ...
        with opendata_engine.connect() as conn:
            ...
            files = [[x.crawl_datafile_key, x.size_in_bytes, x.path, x.endpoint_dbname] for x in
                     endpoint_data.itertuples()]
            ...
            with Pool(processes=NPROC) as pool:
                with tqdm(total=NINPUTS) as pbar:
                    for _ in pool.imap_unordered(process_file_worker,
                        ...
                        if processed % 100 == 0:
                            message_slack(
                                f'Completed {processed} of {NINPUTS} files ({round(100 * processed / NINPUTS, 2)}%) :margarita-parrot:')
                        elif processed % 50 == 0:
                            message_slack(
                                f'Completed {processed} of {NINPUTS} files ({round(100 * processed / NINPUTS, 2)}%) :mask-parrot:')
    ...
    def extract_tables(self, file_dataframe_trimmed, blank_lines, rules_fired=None):
        ...
        while discovered_table != None:
            ...
            if discovered_table != None and set(range(discovered_tables[table_counter - 1]["data_end"] + 1,
                                                      discovered_table["data_start"])).issubset(set(blank_lines + list(
                    set(range(discovered_table["top_boundary"], discovered_table["data_start"])) - set(discovered_table[
                                                                                                           "header"])))):  # discovered_table["data_start"]==discovered_tables[table_counter-1]["data_end"]+1) :
            ...
            if discovered_table != None:
                if table_counter - 1 in discovered_tables.keys() and len(
                        discovered_tables[table_counter - 1]["footnotes"]) > 0:
                    discovered_tables[table_counter - 1]["footnotes"] = list(
                        range(discovered_tables[table_counter - 1]["footnotes"][0],
                ...
                if table_counter - 1 in discovered_tables.keys():
                    ...
                    discovered_tables[table_counter - 1]["data_end_confidence"] = discovered_table[
                        "data_end_confidence"]

                # print(f'->discovered_table={discovered_table}')
                if discovered_table["data_end"] < file_dataframe_trimmed.shape[0] - 1:
                    discovered_table["footnotes"] = list(
                        range(discovered_table["data_end"] + 1, file_dataframe_trimmed.shape[0]))
                else:
                ...
                # print(f'--->discovered_table={discovered_table}')
        if table_counter - 1 in discovered_tables.keys() and discovered_tables[table_counter - 1]["data_end"] != \
                file_dataframe_trimmed.shape[0]:
            footnotes = sorted(list(
                set(range(discovered_tables[table_counter - 1]["data_end"] + 1, file_dataframe_trimmed.shape[0])) - set(
                    blank_lines)))
    ...
    def rules_fired_in_file(self, task):
        ...
        try:
            ...
            data_rules_fired, not_data_rules_fired = collect_dataframe_rules(file_dataframe_trimmed, self, signatures)
            pat_line_datapoints, pat_cell_datapoints, pat_data_line_rules, pat_not_data_line_rules, pat_data_cell_rules, pat_not_data_cell_rules, lines_in_sample = save_training_data(
                datafile_key, file_dataframe_trimmed, annotations, data_rules_fired, not_data_rules_fired, self)
    ...
    def collect_rule_activation(self, db_cred, num_processors, top_level_dir):
        ...
        pat_line_datapoints.head(0).to_sql('pat_line_datapoints', engine, if_exists='replace',
        ...
        engine = create_engine(
            f'postgresql+psycopg2://{db_cred.user}:{db_cred.password}@localhost:{db_cred.port}/{db_cred.database}')
        pat_cell_datapoints.head(0).to_sql('pat_cell_datapoints', engine, if_exists='replace',
                                           index=False)  # truncates the table
        conn = engine.raw_connection()
        cur = conn.cursor()
        output = io.StringIO()
        ...
        engine = create_engine(
            f'postgresql+psycopg2://{db_cred.user}:{db_cred.password}@localhost:{db_cred.port}/{db_cred.database}')
        pat_data_line_rules.head(0).to_sql('pat_data_line_rules', engine, if_exists='replace',
                                           index=False)  # truncates the table
        conn = engine.raw_connection()
        cur = conn.cursor()
        output = io.StringIO()
        ...
        engine = create_engine(
            f'postgresql+psycopg2://{db_cred.user}:{db_cred.password}@localhost:{db_cred.port}/{db_cred.database}')
        pat_not_data_line_rules.head(0).to_sql('pat_not_data_line_rules', engine, if_exists='replace',
                                               index=False)  # truncates the table
        conn = engine.raw_connection()
        cur = conn.cursor()
        output = io.StringIO()
        ...
        engine = create_engine(
            f'postgresql+psycopg2://{db_cred.user}:{db_cred.password}@localhost:{db_cred.port}/{db_cred.database}')
        pat_data_cell_rules.head(0).to_sql('pat_data_cell_rules', engine, if_exists='replace',
                                           index=False)  # truncates the table
        conn = engine.raw_connection()
        cur = conn.cursor()
        output = io.StringIO()
        ...
        engine = create_engine(
            f'postgresql+psycopg2://{db_cred.user}:{db_cred.password}@localhost:{db_cred.port}/{db_cred.database}')
        pat_not_data_cell_rules.head(0).to_sql('pat_not_data_cell_rules', engine, if_exists='replace',
                                               index=False)  # truncates the table
        conn = engine.raw_connection()
        cur = conn.cursor()
        output = io.StringIO()
        ...
        cur.execute(
            """CREATE INDEX IF NOT EXISTS pat_not_data_cell_rules_idx on pat_not_data_cell_rules(crawl_datafile_key)""")

        cur.execute("""CREATE INDEX IF NOT EXISTS pat_data_line_rules_idx on pat_data_line_rules(crawl_datafile_key)""")
        cur.execute(
            """CREATE INDEX IF NOT EXISTS pat_not_data_line_rules_idx on pat_not_data_line_rules(crawl_datafile_key)""")
...
def convert_predictions(predictions, blank_lines, last_line_processed, file_num_columns, file_max_columns_processed, discovered_delimiter):
    ...
    for key in predictions.keys():
        ...
        table["confidence"] = {"body_start": float(predictions[key]["fdl_confidence"]["avg_majority_confidence"]),
                               ...
                               "body": float(combined_table_confidence(
...
def save_training_data(crawl_datafile_key, file_dataframe_trimmed, annotations, data_rules_fired, not_data_rules_fired,
                       pat_model):
    lines_in_sample = 0
    ### LINE###
                ...
                try:
                    for line_index in data_rules_fired.keys():
                        ...
                        if line_index in blank_lines:
                            row_class = "BLANK"
                        elif (len(header_indexes) > 0 and line_index < header_indexes[0]) or (
                                len(header_indexes) == 0 and len(data_indexes) > 0 and line_index < data_indexes[0]):
                            row_class = 'CONTEXT'
                        ...
                        for column_label in file_dataframe_trimmed.columns:
                            ...
                            undersample = False
                            if ('tables' in annotations.keys() and table_counter == 1 and len(annotations[
                                                                                                  'tables']) > 0) and column_label >= pat_model.parameters.ignore_left and (
                                    len(data_indexes) > 0 and data_indexes[
                                0] + pat_model.parameters.undersample_data_limit > line_index) and (
                                    line_index < file_dataframe_trimmed.shape[0] - 1):
                                undersample = True
                            ...
                            disagreement_summary_strength = not_data_rules_fired[line_index][column_label][
                                'disagreement_summary_strength']
                        ...
                        undersample = False
                        if ('tables' in annotations.keys() and table_counter == 1 and len(
                                annotations['tables']) > 0) and (len(data_indexes) > 0 and data_indexes[
                            0] + pat_model.parameters.undersample_data_limit > line_index) and (
                                line_index < file_dataframe_trimmed.shape[0] - 1):
                            undersample = True
                            lines_in_sample += 1
    ...
    pat_line_datapoints = pd.DataFrame(pat_line_datapoints_attribute_values,
                                       columns=pat_line_datapoints_attribute_names)
    pat_cell_datapoints = pd.DataFrame(pat_cell_datapoints_attribute_values,
                                       columns=pat_cell_datapoints_attribute_names)

    pat_data_line_rules = pd.DataFrame(pat_data_line_rules_attribute_values,
                                       columns=pat_data_line_rules_attribute_names)
    pat_not_data_line_rules = pd.DataFrame(pat_not_data_line_rules_attribute_values,
                                           columns=pat_not_data_line_rules_attribute_names)
    pat_data_cell_rules = pd.DataFrame(pat_data_cell_rules_attribute_values,
                                       columns=pat_data_cell_rules_attribute_names)
    pat_not_data_cell_rules = pd.DataFrame(pat_not_data_cell_rules_attribute_values,
    ...
    pat_not_data_cell_rules['disagreement_summary_strength'] = pat_not_data_cell_rules[
        'disagreement_summary_strength'].astype('int')
...
def pat_rule_worker(task):
    ...
    pat_line_datapoints, pat_cell_datapoints, pat_data_line_rules, pat_not_data_line_rules, pat_data_cell_rules, pat_not_data_cell_rules, lines_in_sample = save_training_data(
        crawl_datafile_key, file_dataframe_trimmed,
...
def discover_next_table(csv_file, file_offset, table_counter, data_rules_fired, not_data_rules_fired, blank_lines,
                        headers_discovered, signatures, model):
    ...
    if pat_first_data_line >= 0:
        predicted_pat_header_indexes, candidate_pat_sub_headers = predict_header_indexes(csv_file, pat_first_data_line,
                                                                                         table_counter)
        ...
        aggregation_rows, subheader_scope = predict_subheaders_new(csv_file, cand_data, candidate_pat_sub_headers,
                                                                   blank_lines, predicted_pat_header_indexes, model)
        ...
        discovered_table['fdl_confidence']["avg_confusion_index"] = float(
            first_data_line_combined_data_predictions["avg_confidence"]['confusion_index'])
    ...
    if pat_first_data_line >= 0:
        ...
        candidate_pat_sub_headers = sorted(
            list(set(predicted_pat_sub_headers).intersection(set(predicted_pat_data_lines))))
        ...
        aggregation_scope, subheader_scope = predict_subheaders(csv_file, cand_data, candidate_pat_sub_headers,
                                                                blank_lines, predicted_pat_header_indexes, model)
...
def koci_to_pat_input(crawl_datafile_key, db_cred):
    ...
    for line_index in file_lines:
        ...
        if data_line_weight >= not_data_line_weight:
            ...
            line_predictions[line_index]['value']['confusion_index'] = table_classifier_utilities.confusion_index(
                data_line_weight, not_data_line_weight)
            ...
            line_predictions[line_index]['value']['confusion_index'] = table_classifier_utilities.confusion_index(
                not_data_line_weight, data_line_weight)
...
def predict_subheaders(csv_file, cand_data, predicted_pat_sub_headers, pat_blank_lines, pat_headers, model):
    ...
    for row in cand_data.loc[certain_data_indexes].itertuples():
        ...
        for aggregation_phrase in pat_util.aggregation_functions:
            ...
            if agg_index > -1:
                ...
                aggregation_rows[row.Index]['aggregation_label'] = first_value[:agg_index] + first_value[
                                                                                             agg_index + len(
                                                                                                 aggregation_phrase[
                                                                                                     0]):]
                break
    ...
    aggregation_rows, certain_data_indexes, predicted_pat_sub_headers, cand_subhead_indexes, subheader_scope = discover_aggregation_scope(
        csv_file.loc[:cand_data.index[-1]], aggregation_rows, candidate_subheaders, predicted_pat_sub_headers,
        certain_data_indexes, pat_headers)
    ...
    if cand_subhead_indexes != None and len(cand_subhead_indexes) > 0:
        ...
        for value in first_column_data_values:
            pattern, symbols, case, value_num_tokens, value_num_chars = pat_util.generate_pattern_symbols_and_case(
            ...
            first_column_value_tokens.append([i for i in (str(value).strip().lower()).split() if i not in stop and all(
                j.isalpha() or j in string.punctuation for j in i) and i not in pat_util.null_equivalent_values])
        ...
        if args.normalize_decimals == True:
            first_column_value_patterns, first_column_value_symbols = pat_util.normalize_decimals_numbers(
        ...
        if len(first_column_data_values) > 2:
            candidate_count_for_value = np.count_nonzero(
                first_column_data_values[2:min(args.max_summary_strength, len(first_column_data_values))] == str(
                    value).strip())
        ...
        for rule in fuzzy_rules['cell']['data'].keys():
            ...
            if rule not in ignore_rules['cell']['data'] and len(first_column_value_patterns) > 0:
                ...
                if len(first_column_value_patterns) >= 2 and non_empty_patterns >= 2:
                    rule_fired = eval_data_cell_rule(rule, first_column_data_values, first_column_value_tokens,
                                                     value_pattern_summary, value_chain_consistent,
                                                     value_pattern_BW_summary, value_symbol_summary,
                                                     first_column_value_symbols, first_column_value_patterns,
        ...
        for row in csv_file.loc[cand_subhead_indexes].itertuples():
            ...
            if row.Index - 1 in pat_blank_lines or row.Index - 1 in pat_headers:
                ...
                if args.normalize_decimals == True:
                    column_patterns, column_symbols = pat_util.normalize_decimals_numbers(
                        [pattern] + first_column_value_patterns, [symbols] + first_column_value_symbols)
                ...
                all_patterns_numeric, _ = pat_util.generate_all_numeric_sig_pattern(
                    [table_classifier_utilities.eval_numeric_pattern(pattern)] + first_column_all_patterns_numeric,
                ...
                if len(column_values) > 2:
                    candidate_count_for_value = np.count_nonzero(
                        column_values[2:min(args.max_summary_strength, len(column_values))] == str(value).strip())
                ...
                for rule in fuzzy_rules['cell']['data'].keys():
                    ...
                    if len(column_patterns) > 0 and first_value.lower() not in pat_util.null_equivalent_values:
                        ...
                        if len(column_patterns) >= 2 and non_empty_patterns >= 2:
                            rule_fired = eval_data_cell_rule(rule, column_values, column_tokens, value_pattern_summary,
                                                             value_chain_consistent, value_pattern_BW_summary,
                                                             value_symbol_summary, column_symbols, column_patterns,
                                                             case_summary,
                                                             candidate_count_for_value, partof_multiword_value_repeats,
                                                             candidate_tokens,
                ...
                if len(pattern) > 0:
                    ...
                    for rule in fuzzy_rules['cell']['not_data'].keys():
                        rule_fired = False
                        if rule not in ignore_rules['cell']['not_data'] and disagreement_summary_strength > 0 and (
                                all_numbers(column_symbols) == False or is_number(symbols) == False):
                            rule_fired = eval_not_data_cell_rule(rule, repetitions_of_candidate,
                                                                 repetitions_of_neighbor, neighbor,
                                                                 value_pattern_summary, value_pattern_BW_summary,
                                                                 value_chain_consistent, value_symbol_summary,
                                                                 case_summary, length_summary, pattern, symbols, case,
                                                                 value_num_chars, disagreement_summary_strength,
                                                                 data_rules_fired, 0, 0)
                            if rule_fired == True and "_REPEATS_" not in rule:
                ...
                data_score = max_score(data_rules_fired[0][0]['agreements'], fuzzy_rules['cell']['data'],
                ...
                not_data_score = max_score(value_disagreements, fuzzy_rules['cell']['not_data'],
...
def predict_subheaders_new(csv_file, cand_data, predicted_pat_sub_headers, pat_blank_lines, pat_headers, model):
    ...
    cand_subhead_indexes = list(
        set(predicted_pat_sub_headers + list(cand_data.index[cand_data.iloc[:, 1:].isnull().all(1)])))
    ...
    aggregation_rows, certain_data_indexes, predicted_pat_sub_headers, cand_subhead_indexes, subheader_scope = discover_aggregation_scope(
        csv_file, aggregation_rows, candidate_subheaders, predicted_pat_sub_headers, certain_data_indexes, pat_headers)
        ...
        for row in csv_file.loc[cand_subhead_indexes].itertuples():
            ...
            if row.Index - 1 in pat_blank_lines or row.Index - 1 in pat_headers:
                ...
                for rule in fuzzy_rules['cell']['data'].keys():
                    rule_fired = False
                    non_empty_patterns = 0
                    if rule not in ignore_rules['cell']['data'] and len(
                            column_patterns) > 0 and first_value.lower() not in pat_util.null_equivalent_values:
                        for pattern in column_patterns:
                            if pattern != []:
                                non_empty_patterns += 1
                        if len(column_patterns) >= 2 and non_empty_patterns >= 2:
        ...
        if line_label not in blank_lines:
            ...
            if line_label in predicted_pat_sub_headers:
                ...
                if len(first_value) > 5 and (((first_value[0] == '1' or first_value[0] == 'a') and first_value[1] in [
                    ' ', '.', '/', ')', ']', ':']) or (first_value[0] == '(' and (
                        first_value[1].isdigit() or first_value[1] == 'a') and first_value[2] == ')')):
                    FOOTNOTE_FOUND = True
                    not_data_conf = 1
            ...
            if len(row_values) > 0 and row_values[0].lower() not in ['', 'none', 'nan'] and ((
                                                                                                     len(row_values) > 1 and len(
                                                                                                     [i for i in
                                                                                                      row_values[1:] if
                                                                                                      i.lower() not in [
                                                                                                          '', 'none',
                                                                                                          'nan']]) == 0) or (
                                                                                                     len(row_values) > 2 and len(
                                                                                                     [i for i in
                                                                                                      row_values[2:] if
                                                                                                      i.lower() not in [
                                                                                                          '', 'none',
                ...
                if len(first_value) > 5 and (
                        (first_value[0] == '1' or first_value[0] == 'a') and first_value[1] in [' ', '.', '/', ')', ']',
                                                                                                ':']):
                    FOOTNOTE_FOUND = True

                if len(first_value) > 5 and (
                        first_value[0] == '(' and (first_value[1] == '1' or first_value[1] == 'a') and first_value[
                    2] == ')'):
                    FOOTNOTE_FOUND = True

                if first_value.startswith('(') and len(row_values) > 1 and len(
                        [i for i in row_values[1:] if i != 'nan']) == 0:
                    FOOTNOTE_FOUND = True
            ...
            if line_counter <= 3 or line_label in aggregation_rows:  # and line_predictions[line_label]['label']=='DATA':
                ...
                if line.isnull().values.all() == True:
                    ...
                    for column in candidate_data:
                        ...
                        disagreement_summary_strength = not_data_rules_fired[line_label][column][
                            'disagreement_summary_strength']
                        not_data_score = max_score(value_disagreements, fuzzy_rules["cell"]["not_data"],
                                                   args.not_data_weight_lower_bound)
                        POPULATION_WEIGHT = 1 - (1 - args.p) ** (2 * disagreement_summary_strength)
                        ...
                        if candidate_data.shape[1] > 1:
                            ...
                            for event in not_data_line_rules_fired:
                                if event == "UP_TO_FIRST_COLUMN_COMPLETE_CONSISTENTLY":
                                    continue
                                if event.startswith("ADJACENT_ARITHMETIC_SEQUENCE") and \
                                        fuzzy_rules["line"]["not_data"][event]["weight"] == None:
                                    steps = event[-1]
                                    if steps.isdigit() and int(steps) in range(2, 6):
                                        event = event[:-1] + str(int(steps) + 1)
                                if event in fuzzy_rules["line"]["not_data"].keys() and \
                                        fuzzy_rules["line"]["not_data"][event]["weight"] != None and \
                                        fuzzy_rules["line"]["not_data"][event][
                                            "weight"] > args.not_data_weight_lower_bound:
                    ...
                    if args.weight_input == 'values_and_lines':
                        ...
                        for rule in line_is_data_events:
                            if fuzzy_rules["line"]["data"][rule]["weight"] != None and \
                                    fuzzy_rules["line"]["data"][rule]["weight"] > args.weight_lower_bound:
                    ...
                    elif len(certain_data_widths) > 0 and non_empty_values(line) == max(
                            certain_data_widths) and line_label - 1 in data.index:  # TODO refactor as rule
                        IS_DATA = True
                        # print(f'(4) IS_DATA = True')
                    else:
                        prediction, _ = predict_line_label(data_confidence[line_label], not_data_confidence[line_label])
                        if line_label - 1 not in probation and line_label - 1 not in blank_lines and data_conf < not_data_conf and \
                                prediction['label'] != 'DATA':
                            probation.append(line_label)
                        elif (data_conf > 0 and data_conf >= not_data_conf) or (
                                line_label - 1 not in probation and prediction['label'] == 'DATA'):
                            IS_DATA = True
...
class Patterns:
    ...
    def data_initialize(self, column_index, value, candidate_tokens, column_values, column_tokens, column_trains,
                        column_bw_trains, column_symbols, column_cases, column_char_lengths, column_is_numeric_train,
        ...
        self.data[column_index]['column_is_numeric'] = pat_util.generate_all_numeric_sig_pattern(
            column_is_numeric_train, [len(t) for t in column_trains])
        self.data[column_index][
            'partof_multiword_value_repeats'] = dict()  # TODO check if this contributes to quadratic? ^o^
        for part in candidate_tokens:
    ...
    def data_increment(self, column_index, value, train_sig, bw_train_sig, symbol_sig, case, num_chars_sig,
                       numeric_train_sig, candidate_tokens):

        self.data[column_index]['train'] = pat_util.train_incremental_pattern(self.data[column_index]['train'],
                                                                              train_sig)
        self.data[column_index]['bw_train'] = pat_util.train_incremental_pattern(self.data[column_index]['bw_train'],
                                                                                 bw_train_sig)
        ...
        self.data[column_index]['character_length'] = pat_util.charlength_incremental_pattern(
            self.data[column_index]['character_length'], num_chars_sig)
        self.data[column_index]['summary_strength'] = pat_util.summary_strength_increment(
        ...
        self.data[column_index]['partof_multiword_value_repeats'] = pat_util.token_repeats_increment(
            self.data[column_index]['partof_multiword_value_repeats'], candidate_tokens)
        self.data[column_index]['consistent_symbol_sets'] = pat_util.consistent_symbol_sets_increment(
            self.data[column_index]['consistent_symbol_sets'], symbol_sig)
        self.data[column_index]['column_is_numeric'] = pat_util.numeric_train_incremental_pattern(numeric_train_sig,
                                                                                                  len(train_sig),
                                                                                                  self.data[
                                                                                                      column_index][
                                                                                                      'column_is_numeric'])
    ...
    def not_data_initialize(self, column_index, column_trains, column_bw_trains, column_symbols,
        ...
        if len(train_sig) > 0:
            ...
            try:
                ...
                self.not_data[column_index]['neighbor_count'][neighbor] = np.count_nonzero(
                    context_values[2:] == neighbor)
            except:
    ...
    def not_data_increment(self, column_index, signatures_slice):
        ...
        self.not_data[column_index]['train'] = pat_util.train_incremental_pattern(self.not_data[column_index]['train'],
                                                                                  last_train)
        ...
        self.not_data[column_index]['symbolset'] = pat_util.symbolset_incremental_pattern(
            self.not_data[column_index]['symbolset'], last_symbol_sig)
        self.not_data[column_index]['case'] = pat_util.case_incremental_pattern(self.not_data[column_index]['case'],
                                                                                last_case)
        self.not_data[column_index]['character_length'] = pat_util.charlength_incremental_pattern(
        ...
        if len(train_sig) > 0:
            context_values = signatures_slice.all_normalized_values[1:, column_index]
            self.not_data[column_index]['candidate_count'] = pat_util.candidate_count_increment(
                self.not_data[column_index]['candidate_count'], value)
            neighbor = ''
            try:
                neighbor = context_values[1]
                self.not_data[column_index]['neighbor_count'] = pat_util.candidate_count_increment(
                    self.not_data[column_index]['neighbor_count'], neighbor)
            except:
                self.not_data[column_index]['neighbor_count'] = 0


def collect_line_rules(line, predicted_fdl, line_label, data, signatures, model, line_agreements, line_disagreements,
                       patterns):
    ...
    for column_index, column in enumerate(line.index):
        ...
        is_aggregate = signatures.is_aggregate[
            line_label, column_index]  # (len(value_tokens)>0 and not set(value_tokens).isdisjoint(pat_util.aggregation_tokens))
        is_null_equivalent = signatures.is_null_equivalent[
            line_label, column_index]  # (value_lower in pat_util.null_equivalent_values)
        ...
        if null_equivalent_fired == True or len(value_pattern_summary) > 0 or len(value_pattern_BW_summary) > 0 or len(
                value_symbol_summary) > 0 or len(case_summary) > 0:
...
def get_class_confidences(file_dataframe_trimmed, data_rules_fired, not_data_rules_fired, fuzzy_rules, parameters):
    ...
    for row_index in file_dataframe_trimmed.index:
        ...
        for column_index in column_indexes:
            ...
            if (row_index in data_rules_fired.keys() and data_rules_fired[row_index][column_index][
                'null_equivalent'] == True or data_rules_fired[row_index][column_index][
                    'summary_strength'] == 1) and parameters.impute_nulls == True and row_index - 1 in data_rules_fired.keys() and column_index in \
                    data_rules_fired[row_index - 1].keys() and row_index - 1 in data_line_confidences.keys() and \
                    data_line_confidences[row_index - 1] > not_data_line_confidences[row_index - 1]:
            ...
            if row_index in data_rules_fired.keys() and data_rules_fired[row_index][column_index][
                'summary_strength'] == 0 and data_rules_fired[row_index][column_index][
                'aggregate'] and row_index - 2 in data_rules_fired.keys() and column_index in data_rules_fired[
                row_index - 2].keys() and row_index - 2 in data_line_confidences.keys() and data_line_confidences[
                row_index - 2] > not_data_line_confidences[row_index - 2]:
            ...
            disagreement_summary_strength = not_data_rules_fired[row_index][column_index][
                'disagreement_summary_strength']
            not_data_score = max_score(value_disagreements, fuzzy_rules["cell"]["not_data"],
        ...
        if parameters.weight_input == 'values_and_lines':
            if row_index - 1 in data_line_confidences.keys() and data_line_confidences[row_index - 1] > \
            ...
            if file_dataframe_trimmed.shape[1] > 1:
                ...
                for event in not_data_line_rules_fired:
                    if event == "UP_TO_FIRST_COLUMN_COMPLETE_CONSISTENTLY" and before_data == False:
                        continue
                    if event.startswith("ADJACENT_ARITHMETIC_SEQUENCE") and fuzzy_rules["line"]["not_data"][event][
                        "weight"] == None:
                        steps = event[-1]
                        if steps.isdigit() and int(steps) in range(2, 6):
                            event = event[:-1] + str(int(steps) + 1)
                    if event in fuzzy_rules["line"]["not_data"].keys() and fuzzy_rules["line"]["not_data"][event][
                        "weight"] != None and fuzzy_rules["line"]["not_data"][event][
                        "weight"] > parameters.not_data_weight_lower_bound:
                        line_not_data_evidence.append(fuzzy_rules["line"]["not_data"][event]["weight"])
        ...
        if parameters.weight_input == 'values_and_lines':
            ...
            for rule in line_is_data_events:
                if fuzzy_rules["line"]["data"][rule]["weight"] != None and fuzzy_rules["line"]["data"][rule][
                    "weight"] > parameters.weight_lower_bound:
                    line_is_data_evidence.append(fuzzy_rules["line"]["data"][rule]["weight"])
...
def max_score(events, unit_class_fuzzy_rules, weight_lower_bound):
    if len(events) > 0:
        event_score = []
        for event in events:
            if unit_class_fuzzy_rules[event]["weight"] != None and unit_class_fuzzy_rules[event][
...
def collect_dataframe_rules(csv_file, model, signatures):
    ...
    for row in csv_file.itertuples():
        ...
        for column_index, column in enumerate(csv_file.columns):
                    ...
                    if len(signatures.all_column_train[line_index + nonempty_patterns_idx, column_index]) > 0:
                        ...
                        if nonempty_patterns == args.max_summary_strength:
                            ...
                            column_symbols = signatures.all_column_symbols[
                                             line_index:line_index + nonempty_patterns_idx + 1, column_index]
            ...
            if null_equivalent_fired == True or len(value_pattern_summary) > 0 or len(
                    value_pattern_BW_summary) > 0 or len(value_symbol_summary) > 0 or len(case_summary) > 0:
                all_summaries_empty = False
            ...
            for rule in fuzzy_rules["cell"]["data"].keys():
                ...
                # CHECK RULE
                if rule not in ignore_rules["cell"]["data"] and len(
                        column_train_sigs) > 0 and value_lower not in pat_util.null_equivalent_values:
                    ...
                    if (len(column_train_sigs) >= 2 and non_empty_patterns >= 2) or (
                            len(csv_file.index) > 0 and line_index == csv_file.index[-1]):  ### TEST CHANGE
                        assert (len(column_values) > 0)

                        rule_fired = eval_data_cell_rule(rule, column_values, column_tokens, value_pattern_summary,
                                                         value_chain_consistent, value_pattern_BW_summary,
                                                         value_symbol_summary, column_symbols, column_train_sigs,
                                                         case_summary, candidate_count_for_value,
                                                         partof_multiword_value_repeats,
                                                         candidate_tokens, consistent_symbol_sets,
                                                         train_sigs_all_numeric,
    ...
    for row in csv_file.itertuples():
                    ...
                    if len(signatures.all_column_train[line_index + 1 + nonempty_patterns_idx, columnindex]) > 0:
                        ...
                        if nonempty_patterns == args.max_summary_strength:
                            column_train_sigs = signatures.train_normalized_numbers[
                                                line_index + 1:line_index + 1 + nonempty_patterns_idx + 1,
                                                columnindex].tolist()
                            ...
                            column_symbols = signatures.symbolset_normalized_numbers[
                                             line_index + 1:line_index + 1 + nonempty_patterns_idx + 1, columnindex]
                            column_cases = signatures.all_column_cases[
                                           line_index + 1:line_index + 1 + nonempty_patterns_idx + 1, columnindex]
                            column_lengths = signatures.all_column_character_lengths[
                                             line_index + 1:line_index + 1 + nonempty_patterns_idx + 1, columnindex]
            ...
            for rule in fuzzy_rules["cell"]["not_data"].keys():
                ...
                if rule not in ignore_rules["cell"]["not_data"] and len(cand_pattern) > 0:
                    if disagreement_summary_strength > 0 and (
                            np.all(signatures.all_column_isnumber[line_index:, columnindex]) == False):
                        rule_fired = eval_not_data_cell_rule(rule,
        ...
        header_row_with_aggregation_tokens_fired = header_row_with_aggregation_tokens(row_values,
                                                                                      arithmetic_sequence_fired)

        before_data = True
        not_data_line_rules_fired = []
        if csv_file.shape[1] > 1:
            not_data_line_rules_fired = assess_non_data_line(row_values, before_data, all_summaries_empty, line_index,
                                                             csv_file)

        for rule in fuzzy_rules["line"]["not_data"].keys():
            rule_fired = False
            if rule not in ignore_rules["line"]["not_data"] and rule in (
                    not_data_line_rules_fired + header_events_fired + arithmetic_events_fired + header_row_with_aggregation_tokens_fired):
                rule_fired = True
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--filepath",
                        default=None)  # , description="Filepath to CSV file over which to infer annotations")
    parser.add_argument("-o", "--output_file", default=None)
    parser.add_argument("-c", "--csv_files",
                        default=None)  # , description="Filepath to folder with CSV training files")
    parser.add_argument("-a", "--annotations",
                        default=None)  # , description="Filepath to folder with JSON annotations over CSV training files")
